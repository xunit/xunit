...
namespace Xunit.Runner.MSBuild
{
    public class TeamCityVisitor : MSBuildVisitor
    {
        ...
        private readonly TeamCityDisplayNameFormatter displayNameFormatter;

        public TeamCityVisitor(TaskLoggingHelper log, XElement assemblyElement, Func<bool> cancelThunk)
            : this(log, assemblyElement, cancelThunk, _ => Guid.NewGuid().ToString("N"), new TeamCityDisplayNameFormatter()) { }

        public TeamCityVisitor(TaskLoggingHelper log, XElement assemblyElement, Func<bool> cancelThunk, TeamCityDisplayNameFormatter displayNameFormatter)
            : this(log, assemblyElement, cancelThunk, _ => Guid.NewGuid().ToString("N"), displayNameFormatter) { }

        public TeamCityVisitor(TaskLoggingHelper log, XElement assemblyElement, Func<bool> cancelThunk, Func<string, string> flowIdMapper, TeamCityDisplayNameFormatter displayNameFormatter = null)
