...
public static class CodeGenTestAssemblyRunnerTests
{
		...
		public static async ValueTask Passing()
		{
			...
			Assert.Collection(
				...
				msg =>
				{
					...
					Assert.Matches($"^{IntPtr.Size * 8}-bit \\({Regex.Escape(RuntimeInformation.ProcessArchitecture.ToDisplayName())}\\) {Regex.Escape(RuntimeInformation.FrameworkDescription)} \\[collection-per-class, parallel Assemblies, Collections \\(\\d+ threads\\)\\]$", starting.TestEnvironment);
	...
	public static class Fixtures
	{
		...
		{
			var testAssembly = Mocks.CodeGenTestAssembly(assemblyFixtureFactories: new Dictionary<Type, FixtureFactory>()
			{
		...
		public static async ValueTask FixtureEvents()
		{
			...
			var testAssembly = Mocks.CodeGenTestAssembly(assemblyFixtureFactories: factories);
			var testCollection = Mocks.CodeGenTestCollection(testAssembly: testAssembly);
