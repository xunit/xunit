namespace Xunit.Generators;

partial class Types
{
	partial class Xunit
	{
		public const string AssemblyFixtureAttribute = "Xunit.AssemblyFixtureAttribute";
		public const string CaptureConsoleAttribute = "Xunit.CaptureConsoleAttribute";
		public const string CaptureTraceAttribute = "Xunit.CaptureTraceAttribute";
		public const string ClassDataAttribute = "Xunit.ClassDataAttribute";
		public const string CollectionBehaviorAttribute = "Xunit.CollectionBehaviorAttribute";
		public const string CulturedFactAttribute = "Xunit.CulturedFactAttribute";
		public const string CulturedTheoryAttribute = "Xunit.CulturedTheoryAttribute";
		public const string FactAttribute = "Xunit.FactAttribute";
		public const string IAsyncLifetime = "Xunit.IAsyncLifetime";
		public const string InlineDataAttribute = "Xunit.InlineDataAttribute";
		public const string MemberDataAttribute = "Xunit.MemberDataAttribute";
		public const string TestFrameworkAttribute = "Xunit.TestFrameworkAttribute";
		public const string TheoryAttribute = "Xunit.TheoryAttribute";

		partial class v3
		{
			public const string ParallelizationAttribute = "Xunit.v3.ParallelizationAttribute";
			public const string TestPipelineStartupAttribute = "Xunit.v3.TestPipelineStartupAttribute";
		}
	}
}
