	...
	where TTestCase : class, ITestCase
{
	...
	protected virtual ValueTask<bool> OnError(
		TContext ctxt,
		Exception exception) =>
			new(Guard.ArgumentNotNull(ctxt).MessageBus.QueueMessage(ErrorMessage.FromException(exception, ctxt.TestAssembly.UniqueID)));
	...
	/// <summary>
	/// This method will be called before the test assembly has started running. By default this sends
	...
	protected virtual async ValueTask<bool> OnTestAssemblyStarting(TContext ctxt)
	{
		...
		{
			AssemblyName = ctxt.TestAssembly.SimpleAssemblyName(),
