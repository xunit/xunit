...
public sealed class _ErrorMessage : _MessageSinkMessage, _IErrorMetadata
{
	...
	protected override void Deserialize(IReadOnlyDictionary<string, object?> root)
	{
		...
		if (JsonDeserializer.TryGetArrayOfInt(root, nameof(_IErrorMetadata.ExceptionParentIndices)) is int[] expectedParentIndices)
			ExceptionParentIndices = expectedParentIndices;
		if (JsonDeserializer.TryGetArrayOfNullableString(root, nameof(_IErrorMetadata.ExceptionTypes)) is string?[] exceptionTypes)
			ExceptionTypes = exceptionTypes;
		if (JsonDeserializer.TryGetArrayOfString(root, nameof(_IErrorMetadata.Messages)) is string?[] messages)
			Messages = messages;
		if (JsonDeserializer.TryGetArrayOfNullableString(root, nameof(_IErrorMetadata.StackTraces)) is string?[] stackTraces)
