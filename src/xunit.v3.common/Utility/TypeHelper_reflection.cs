...
partial class TypeHelper
{
	...
	public static Type? GetType(string assemblyQualifiedTypeName) =>
		typeCache.GetOrAdd(
			...
			aqtn =>
			{
				var firstOpenSquare = aqtn.IndexOfOrdinal('[');
				if (firstOpenSquare > 0)
				{
					var backtick = aqtn.IndexOfOrdinal('`');
...
#if NETCOREAPP
							var genericDefinitionName = string.Concat(aqtn.AsSpan(0, firstOpenSquare), aqtn.AsSpan(lastOpenSquare));
#else
