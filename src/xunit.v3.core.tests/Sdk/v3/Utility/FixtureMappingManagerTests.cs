...
public class FixtureMappingManagerTests
{
	...
	{
		public FixtureWithDependency(object dependency) => Dependency = dependency;
	...
	{
		public FixtureWithMessageSinkAndTestContext(_IMessageSink messageSink, ITestContextAccessor contextAccessor)
		{
			MessageSink = messageSink;
			ContextAccessor = contextAccessor;
		}

		public ITestContextAccessor ContextAccessor { get; }
		public _IMessageSink MessageSink { get; }
	}
	...
	public async ValueTask FixtureWithThrowingDisposeThrows()
	{
		...
		Assert.IsType<TestFixtureCleanupException>(ex);
		Assert.Equal("Testable fixture type 'FixtureMappingManagerTests+FixtureWithThrowingDispose' threw in Dispose", ex.Message);
		Assert.IsType<DivideByZeroException>(ex.InnerException);
	}
	...
	public async ValueTask FixtureWithThrowingDisposeAsyncThrows()
	{
		...
		Assert.IsType<TestFixtureCleanupException>(ex);
		Assert.Equal("Testable fixture type 'FixtureMappingManagerTests+FixtureWithThrowingDisposeAsync' threw in DisposeAsync", ex.Message);
		Assert.IsType<DivideByZeroException>(ex.InnerException);
	}
