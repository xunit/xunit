...
public class XunitDelayEnumeratedTheoryTestCaseRunnerTests
{
	...
	public static async ValueTask EnumeratesDataAtRuntimeAndExecutesOneTestForEachDataRow()
	{
		...
		var passed = messages.OfType<_TestPassed>().Single();
		var passedStarting = messages.OfType<_TestStarting>().Single(ts => ts.TestUniqueID == passed.TestUniqueID);
		Assert.Equal($"Display Name(x: 42, _1: {21.12:G17}, _2: \"Hello\")", passedStarting.TestDisplayName);
		var failed = messages.OfType<_TestFailed>().Single();
		var failedStarting = messages.OfType<_TestStarting>().Single(ts => ts.TestUniqueID == failed.TestUniqueID);
	...
	public static async ValueTask SkipsDataAttributesWithSkipReason()
	{
		...
		Assert.Collection(
			messages.OfType<_TestPassed>().Select(p => messages.OfType<_TestStarting>().Single(s => s.TestUniqueID == p.TestUniqueID).TestDisplayName).OrderBy(x => x),
			displayName => Assert.Equal($"Display Name(x: 1, _1: {2.1:G17}, _2: \"not skipped\")", displayName)
		);
		Assert.Collection(
			messages.OfType<_TestFailed>().Select(p => messages.OfType<_TestStarting>().Single(s => s.TestUniqueID == p.TestUniqueID).TestDisplayName).OrderBy(x => x),
		...
		Assert.Collection(
			messages.OfType<_TestSkipped>().Select(p => messages.OfType<_TestStarting>().Single(s => s.TestUniqueID == p.TestUniqueID).TestDisplayName).OrderBy(x => x),
	...
	public async ValueTask ThrowingToString()
	{
		...
		var passed = runner.MessageBus.Messages.OfType<_TestPassed>().Single();
		var passedStarting = runner.MessageBus.Messages.OfType<_TestStarting>().Where(ts => ts.TestUniqueID == passed.TestUniqueID).Single();
