...
public class InlineDataAcceptanceTests : AcceptanceTest
{
    ...
    public void MultipleDataSets()
    {
        ...
        Assert.Equal(@"InlineDataAcceptanceTests+MultipleDataSetsClass.PassingTestData(foo: 1, bar: ""hello"", baz: 2.3)", passedResult.DisplayName);
        FailedResult failedResult = results.OfType<FailedResult>().Single();
        Assert.Equal(@"InlineDataAcceptanceTests+MultipleDataSetsClass.PassingTestData(foo: 42, bar: ""world"", baz: 21.12)", failedResult.DisplayName);
    }
    ...
    public void NullValue()
    {
        ...
        Assert.Equal(@"InlineDataAcceptanceTests+NullValueClass.PassingTestData(foo: null)", result.DisplayName);
    }
