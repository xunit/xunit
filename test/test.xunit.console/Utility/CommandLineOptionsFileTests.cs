...
public class CommandLineOptionsFileReaderTests
{
    ...
    class TestableCommandLineOptionsFile : CommandLineOptionsFile
    {
        ...
        {
            Console.WriteLine("in TestableCommandLineOptionsFile Read(string optionsFile)");
            Console.WriteLine("optionsFile: " + optionsFile);
            return new TestableCommandLineOptionsFile(optionsFile, new StreamReader(new MemoryStream(Encoding.UTF8.GetBytes(string.Empty))));
        }

        public static new TestableCommandLineOptionsFile Read(string optionsFile, string fileContents)
        {
            Console.WriteLine("in TestableCommandLineOptionsFile Read(string optionsFile, string fileContents)");
            Console.WriteLine("optionsFile: " + optionsFile);
            Console.WriteLine("fileContents: " + fileContents);
            return new TestableCommandLineOptionsFile(optionsFile, new StreamReader(new MemoryStream(Encoding.UTF8.GetBytes(fileContents ?? string.Empty))));
        }
    }
}
