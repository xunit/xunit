...
function _Global-Setup {
  ...
  New-Item -type directory -path $cmdBinPath -Force | Out-Null
...
param(
  ...
  if(Test-Path $runtimeTempDownload) {
    ...
    New-Item -type directory -path $runtimeTempDownload -Force | Out-Null
  }
  ...
  New-Item -type directory -path $runtimeFolder -Force | Out-Null
  ...
  if ($versionOrAlias.EndsWith(".nupkg")) {
    ...
    if ($folderExists) {
      ...
      if(Test-Path $tempUnpackFolder) {
          ...
          New-Item -type directory -path $tempUnpackFolder -Force | Out-Null
      }
...
function _List {
  ...
  New-Item -type directory -path ($userHome + "\alias\") -Force | Out-Null
...
param(
  ...
  New-Item -type directory -path $userHome -Force | Out-Null
  $env:Path = $newPath
@"
SET "PATH=$newPath"
